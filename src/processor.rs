...
use crate::decoder::functions::accumulator_to_memory::accumulator_to_memory;
use crate::decoder::functions::handle_jumps_loops::handle_jumps_loops;
use crate::decoder::functions::immediate_to_accumulator::immediate_to_accumulator;
use crate::decoder::functions::immediate_to_register::immediate_to_register;
use crate::decoder::functions::immediate_to_register_memory::immediate_to_register_memory;
use crate::decoder::functions::memory_to_accumulator::memory_to_accumulator;
use crate::decoder::functions::register_memory_to_from_register::register_memory_to_from_register;
    ...
    while let Some(instruction) = contents_iterator.next() {
        ...
        match (opcode, opcode_instruction) {
            ...
            (Op::Je, Instruction::JumpOnEqual) => {
                handle_jumps_loops(Op::Je, &mut contents_iterator, &mut output);
            }
            (Op::Jl, Instruction::JumpOnLess) => {
                handle_jumps_loops(Op::Jl, &mut contents_iterator, &mut output);
            }
            (Op::Jle, Instruction::JumpOnLessOrEqual) => {
                handle_jumps_loops(Op::Jle, &mut contents_iterator, &mut output);
            }
            (Op::Jb, Instruction::JumpOnBelow) => {
                handle_jumps_loops(Op::Jb, &mut contents_iterator, &mut output);
            }
            (Op::Jbe, Instruction::JumpOnBelowOrEqual) => {
                handle_jumps_loops(Op::Jbe, &mut contents_iterator, &mut output);
            }
            (Op::Jp, Instruction::JumpOnParity) => {
                handle_jumps_loops(Op::Jp, &mut contents_iterator, &mut output);
            }
            (Op::Jo, Instruction::JumpOnOverflow) => {
                handle_jumps_loops(Op::Jo, &mut contents_iterator, &mut output);
            }
            (Op::Js, Instruction::JumpOnSign) => {
                handle_jumps_loops(Op::Js, &mut contents_iterator, &mut output);
            }
            (Op::Jne, Instruction::JumpOnNotEqual) => {
                handle_jumps_loops(Op::Jne, &mut contents_iterator, &mut output);
            }
            (Op::Jnl, Instruction::JumpOnNotLess) => {
                handle_jumps_loops(Op::Jnl, &mut contents_iterator, &mut output);
            }
            (Op::Jg, Instruction::JumpOnGreater) => {
                handle_jumps_loops(Op::Jg, &mut contents_iterator, &mut output);
            }
            (Op::Jnb, Instruction::JumpOnNotBelow) => {
                handle_jumps_loops(Op::Jnb, &mut contents_iterator, &mut output);
            }
            (Op::Ja, Instruction::JumpOnAbove) => {
                handle_jumps_loops(Op::Ja, &mut contents_iterator, &mut output);
            }
            (Op::Jnp, Instruction::JumpOnNotPar) => {
                handle_jumps_loops(Op::Jnp, &mut contents_iterator, &mut output);
            }
            (Op::Jno, Instruction::JumpOnNotOverflow) => {
                handle_jumps_loops(Op::Jno, &mut contents_iterator, &mut output);
            }
            (Op::Jns, Instruction::JumpOnNotSign) => {
                handle_jumps_loops(Op::Jns, &mut contents_iterator, &mut output);
            }
            (Op::Loop, Instruction::LoopCxTimes) => {
                handle_jumps_loops(Op::Loop, &mut contents_iterator, &mut output);
            }
            (Op::Loopz, Instruction::LoopWhileZero) => {
                handle_jumps_loops(Op::Loopz, &mut contents_iterator, &mut output);
            }
            (Op::Loopnz, Instruction::LoopWhileNotZero) => {
                handle_jumps_loops(Op::Loopnz, &mut contents_iterator, &mut output);
            }
            (Op::Jcxz, Instruction::JumpOnCxZero) => {
                handle_jumps_loops(Op::Jcxz, &mut contents_iterator, &mut output);
            }
            (_, _) => (),
        }
    }
